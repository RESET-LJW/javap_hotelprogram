...
public class Request {
	...
	public void asdfasdf(String name, String phoneNumber) {
		int count = 0;
		for (int i = 0; i < room.length; i++) {
			for (int j = 0; j < room[i].length; j++) {
				int a = room[i][j].guestState(name, phoneNumber);
				if (a == 1) {
					System.out.printf("%s 고객님의 정보로 예약된 방은 %d%02d호 입니다.\n", name, i + 2, j + 1);
					count++;
				} else if (a == 2) {
					System.out.printf("%s 고객님의 정보로 투숙중인 방은 %d%02d호 입니다.\n", name, i + 2, j + 1);
					count++;
				}
			}
		}
		if (count == 0) {
			System.out.println("해당 고객님의 정보로 예약,투숙 중인 방이 없습니다.");
		}
	}
}
